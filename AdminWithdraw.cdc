// Import FungibleToken, FlowToken, and GreenToken from version 0x05

transaction(senderAccount: Address, amount: UFix64) {

    // Define references
    let senderVault: &GreenToken.Vault{GreenToken.VaultInterface}
    ...
    let adminResource: &GreenToken.Admin
    let flowMinter: &FlowToken.Minter

    prepare(acct: AuthAccount) {
        // Borrow references and handle errors
        self.adminResource = acct.borrow<&GreenToken.Admin>(from: /storage/AdminStorage)
        ...
        self.senderVault = getAccount(senderAccount)
            ...
            .borrow<&GreenToken.Vault{GreenToken.VaultInterface}>()
