// Import the FungibleToken library version 0x05
import FungibleToken from 0x05

// GreenToken contract: Implements a fungible token on the Flow blockchain
pub contract GreenToken: FungibleToken {

    // Total supply of GreenToken
    ...
    pub resource Vault: FungibleToken.Provider, FungibleToken.Receiver, FungibleToken.Balance, VaultInterface {
        ...
        pub fun deposit(from: @FungibleToken.Vault) {
            let vault <- from as! @GreenToken.Vault
        ...
        // Destroy the vault
        destroy() {
            GreenToken.totalSupply = GreenToken.totalSupply - self.balance
        }
    }
    ...
    pub resource Minter {
        ...
        pub fun mintToken(amount: UFix64): @FungibleToken.Vault {
            GreenToken.totalSupply = GreenToken.totalSupply + amount
            return <-create Vault(balance: amount)
        }
    }
    ...
    init() {
        ...
        self.account.link<&GreenToken.Minter>(/public/Minter, target: /storage/MinterStorage)
